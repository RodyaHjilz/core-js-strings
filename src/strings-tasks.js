 ...
 * @return {string} - The striыекng with the first occurrence of the substring removed.
 *
 * @example
...
function countVowels(str) {
  function countV(sub, value) {
    let count = 0;
    let position = sub.indexOf(value);

    while (position !== -1) {
      count += 1;
      position = sub.indexOf(value, position + 1);
    }
    return count;
  }
  const sub = str.toLowerCase();
  const sum =
    countV(sub, 'a') +
    countV(sub, 'e') +
    countV(sub, 'i') +
...
function isPalindrome(str) {
  const sub = str.replace(/[^a-zA-Z0-9]/g, '').toLowerCase();
  return reverseString(sub) === sub;
}
...
function reverseWords(str) {
  ...
  let result = '';
  for (let i = 0; i < arr.length; i += 1) {
    result += reverseString(arr[i]).concat(' ');
  }
...
function encodeToRot13(str) {
  ...
  const encodedArray = str.split('').map((char) => {
    if (/[A-Za-z]/.test(char)) {
